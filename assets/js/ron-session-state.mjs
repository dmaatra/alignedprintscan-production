...
export function buildRonSessionRows(payload={}) {
  ...
  return requests.map(request=>{
    ...
    row.attention=[row.payment.key!=="paid",row.appointment.key!=="confirmed",row.signers.key!=="ready",row.documents.key!=="ready",["needs_attention","completed_with_rejections"].includes(row.proof.key),["retrieved","pending_review"].includes(row.documentReturn.key)].some(Boolean);
    row.sessionStatus=row.documentReturn.key==="released"?badge("released","Released"):row.attention?badge("needs_attention","Needs Attention"):row.proof.key==="completed"?badge("completed","Completed"):["activated","in_progress"].includes(row.proof.key)?badge("active","Active / In Progress"):row.proof.key==="ready_for_activation"?badge("ready","Ready"):badge("preparing","Preparing");
