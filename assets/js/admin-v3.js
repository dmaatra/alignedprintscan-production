...
(() => {
  ...
  function renderTemplates() {
    ...
    const rows=moduleState.templates.filter(template=>{const haystack=`${template.template_key} ${template.name} ${template.description||""}`.toLowerCase(),service=templateService(template);return(!moduleState.templateView.search||haystack.includes(moduleState.templateView.search.toLowerCase()))&&(moduleState.templateView.category==="all"||templateCategory(template)===moduleState.templateView.category)&&(moduleState.templateView.service==="all"||service===moduleState.templateView.service);});
  ...
  function renderModule(view) {
    ...
    if(view==="staff-access") return renderStaffAccess();
