...
(() => {
  ...
  function preserveAdminDynamicValues(host, render) {
    const values = new Map($$("input[name], select[name], textarea[name]", host).map(control => [control.name, control.type === "checkbox" ? control.checked : control.value]));
    render();
    $$("input[name], select[name], textarea[name]", host).forEach(control => {
  ...
  function setWizardRonStructuredFields(form) {
    ...
    if(Number(signerHost.dataset.count)!==signerCount){signerHost.dataset.count=String(signerCount);preserveAdminDynamicValues(signerHost,()=>{signerHost.innerHTML=`<h3>Structured signers</h3>${Array.from({length:signerCount},(_,index)=>`<fieldset><legend>Signer ${index+1} legal ID name</legend><div class="admin-v3-form-grid"><label>First name<input name="ron_signer_first_${index}" required></label><label>Middle name (optional)<input name="ron_signer_middle_${index}"></label><label>Last name<input name="ron_signer_last_${index}" required></label><label>Individual email<input name="ron_signer_email_${index}" type="email" required></label><label>Phone<input name="ron_signer_phone_${index}" type="tel"></label></div></fieldset>`).join("")}`;});}
    if(Number(actHost.dataset.count)!==actCount){actHost.dataset.count=String(actCount);preserveAdminDynamicValues(actHost,()=>{actHost.innerHTML=`<h3>Requested notarial acts</h3>${Array.from({length:actCount},(_,index)=>`<label>Act ${index+1}<select name="ron_act_type_${index}" required><option value="acknowledgment">Acknowledgment</option><option value="jurat">Jurat / verification on oath</option><option value="signature_witnessing">Signature witnessing</option><option value="certified_copy">Certified copy (when permitted)</option><option value="unsure">I’m not sure</option></select></label>`).join("")}`;});}
  ...
  function setWizardMobileStructuredFields(form) {
    ...
    if(Number(actHost.dataset.count)!==actCount){actHost.dataset.count=String(actCount);preserveAdminDynamicValues(actHost,()=>{actHost.innerHTML=`<h3>Requested notarial acts</h3>${Array.from({length:actCount},(_,index)=>`<label>Act ${index+1}<select name="mobile_act_type_${index}" required><option value="acknowledgment">Acknowledgment</option><option value="jurat">Jurat / verification on oath</option><option value="signature_witnessing">Signature witnessing</option><option value="certified_copy">Certified copy (when permitted)</option><option value="unsure">I’m not sure</option></select></label>`).join("")}`;});}
