...
(() => {
  ...
  function bindRelease2ManagementActions() {
    ...
    $$('.organization-card',moduleContent).forEach((card,index)=>{const org=moduleState.businessFoundation.organizations[index];if(org)card.insertAdjacentHTML('beforeend',`<div class="admin-v3-module-grid">${portalPreviewMarkup(org)}${closureReviewMarkup(org)}</div>`);});
    ...
    $$('.organizationPolicyForm',moduleContent).forEach(form=>form.addEventListener('submit',async event=>{event.preventDefault();const body=businessFormBody(form);for(const key of ['credit_hold','service_ron_enabled','service_mobile_enabled','service_print_enabled','service_loan_signing_enabled'])body[key]=form.elements[key].checked;try{await businessCommand('save_organization',{organization_id:form.dataset.organizationId,...body});await redrawOrganizations();}catch(error){alert(error.message);}}));
