...
async function loadLoanSigningFulfillmentPanel(){const panel=$("#loanSigningFulfillmentPanel");if(!panel||selectedRequest?.service_type!=="loan_signing")return;try{const data=await invokeLoanSigningFulfillment("snapshot"),a=data.assignment||{},active=(data.packages||[]).find(p=>p.status==="active"),blockers=data.completion?.blockers||[],exceptions=data.exceptions||[],charges=data.charges||[],financials=data.financials||{};panel.innerHTML=`
  ...
  ${exceptions.map(x=>`<article class="admin-v3-overview-card"><span class="small-label">${escapeHtml(statusLabel(x.status))}</span><strong>${escapeHtml(statusLabel(x.outcome))}</strong><p>${escapeHtml(x.customer_safe_status||"")}</p>${!["resolved","closed"].includes(x.status)?`<button class="btn dark resolve-lsa-financial" data-id="${x.id}" type="button">Authorize Financial Resolution</button><button class="btn dark add-lsa-visit" data-id="${x.id}" type="button">Add Linked Visit</button>`:""}</article>`).join("")||"<p>No exception records.</p>"}
  ...
  $$(".resolve-lsa-financial",panel).forEach(button=>button.addEventListener("click",async()=>{const resolution_type=prompt("Resolution: no_charge, partial_charge, full_agreed_fee, custom_authorized_amount, full_refund, partial_refund, no_refund, or no_financial_change","no_financial_change");if(!resolution_type)return;const authorized_charge=Number(prompt("Authorized cancellation/service charge", "0")||0),refund_due=Number(prompt("Refund due (processing remains in Refund SOP)","0")||0),additional_amount_due=Number(prompt("Additional amount due (invoice remains separate)","0")||0),decision_reason=prompt("Required internal decision rationale:");if(!decision_reason)return;const customer_safe_explanation=prompt("Customer-safe explanation (no blame or internal policy matrix):")||"APS completed its review and will provide any required financial follow-up separately.";try{await invokeLoanSigningFulfillment("resolve_financial",{exception_id:button.dataset.id,resolution_type,authorized_charge,refund_due,additional_amount_due,decision_reason,customer_safe_explanation});await loadLoanSigningFulfillmentPanel()}catch(error){alert(error.message)}}));
