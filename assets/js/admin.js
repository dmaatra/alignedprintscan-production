...
async function loadLoanSigningFulfillmentPanel(){const panel=$("#loanSigningFulfillmentPanel");if(!panel||selectedRequest?.service_type!=="loan_signing")return;try{const data=await invokeLoanSigningFulfillment("snapshot"),a=data.assignment||{},active=(data.packages||[]).find(p=>p.status==="active"),blockers=data.completion?.blockers||[];panel.innerHTML=`
