...
function receiptPanel(request, reference){
  ...
  return `<div class="next-panel receipt-panel reveal"><h3>${submitted?'Payment Submitted':'Payment Receipt'}</h3><p>${submitted?'Thank you. Your secure payment was completed through Stripe. Aligned Print & Scan has been notified and will send your official payment-received confirmation after admin review.':'Thank you. Your payment has been received and recorded for your request.'}</p><div class="request-public-detail-grid"><div><span class="small-label">Reference</span><strong>${escapePublic(reference)}</strong></div><div><span class="small-label">Amount Paid</span><strong>${money(amount)}</strong></div><div><span class="small-label">Paid On</span><strong>${escapePublic(paidDate)}</strong></div></div><div class="cta-row"><button class="btn dark" type="button" onclick="window.print()">Print Receipt / Confirmation</button><a class="btn secondary visible-secondary" href="support.html?ref=${encodeURIComponent(reference)}">Questions? Contact Support</a></div></div>`;
}
