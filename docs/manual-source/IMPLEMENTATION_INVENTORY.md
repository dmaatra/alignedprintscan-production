...
GA4 is active and production-verified with the owner-supplied public Measurement ID `G-4KXRE49B0B`. It initializes once on the maintained analytics path, uses an allowlisted/deduplicated event taxonomy, disables advertising signals, and sends only sanitized service category and canonical path context. The Search Console URL-prefix property `https://alignedprintscan.com/` is verified by the permanent HTML file `google7bace5a38d37ffed.html`, and `https://alignedprintscan.com/sitemap.xml` was submitted successfully. Admin, customer-request, document, Proof, and other private routes remain excluded from sitemap/indexing eligibility. Owner Google setup remaining: none. The APS database remains authoritative for request, quote, payment, completion, service, attribution, and revenue relationships; GA4 measures public traffic and approved funnel interactions only.
