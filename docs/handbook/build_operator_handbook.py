...
("VIII","SCHEDULING, RESCHEDULING & CANCELLATION",["Scheduling and Confirmation","Rescheduling","Changing Service Type / RON to Mobile Conversion","Cancellation Review","No-show, Late Cancellation, and APS Unable to Fulfill"]),
...
def add_chapter(doc,part_roman,chapter_no,title,anchor):
    ...
    add_refinement_guidance(doc,title,anchor)
def add_refinement_guidance(doc,title,anchor):
    if title=="Customer":
        add_heading(doc,"Structured signer identity and readiness",3,anchor+"_structured_signers")
        add_bullets(doc,["Record every signer with a required first name and last name. Middle name is optional and must never be invented.","Display First Middle Last when a middle name exists, otherwise First Last.","RON requires each signer's individual email; Mobile requires the identity details needed for the appointment; Print & Scan does not require a notarial signer.","A Review Queue item must name the missing field. Correcting the applicable data clears the blocker; completed requests do not retain obsolete participant blockers."])
    elif title=="Overview":
        add_heading(doc,"Service-detail presentation",3,anchor+"_service_detail_terms")
        add_table(doc,["Operator label","Meaning / display rule"],[("Completed-document scan back","APS scans a completed or signed physical document for digital return. Display Yes or No."),("Document scanning / PDF conversion","Physical pages are converted to a PDF independent of completed-document scan-back. Display Yes or No."),("Mobile Location","Show street/unit and city, actual stored state, and ZIP together; keep structured fields for validation and pricing."),("Witnesses not required","Show Required: No; Total: 0; Provider/customer/APS provider fields: N/A; Review Required: No.")],[2.2,4.3])
    elif title=="Communication Log and Timeline":
        add_heading(doc,"Inbound upload correspondence and Activity",3,anchor+"_inbound_upload")
        add_bullets(doc,["A customer-uploaded file remains a Document with its provenance.","An accompanying customer message is a separate Inbound · Customer Communication Log entry and references the uploaded attachment.","Customer Activity records Documents received automatically; Document Received / Under Review remains an optional manual email.","Appointment confirmation and service conversion create persistent, human-readable customer Activity events."])
    elif title=="Template Operator Workflow":
        add_heading(doc,"New manual templates",3,anchor+"_refinement_templates")
        add_table(doc,["Template","Use","Automatic?"],[("Document Needed to Complete Quote","Admin cannot finalize an accurate quote without the relevant document.","No - manual decision."),("Document Received / Under Review","Optional reassurance after a document is supplied.","No - Activity is the automatic acknowledgment."),("Service Changed / Appointment Conversion","Explain RON/Mobile conversion, scheduling impact, financial effect, and next action.","No - send after approved conversion.")],[2.25,3.55,.7])
    elif title=="Stripe and Manual Payment":
        add_heading(doc,"Payment Received transaction semantics",3,anchor+"_payment_binding")
        add_table(doc,["Field","Authoritative value"],[("Invoice","Invoice linked to the payment that triggered the communication."),("Payment Recorded","That single new payment amount."),("Payment Date","That single payment's received/created date."),("Paid to Date","Cumulative payments recorded for the request."),("Balance Due","Current legitimate balance after the payment.")],[1.75,4.75])
        add_callout(doc,"Final payment sequence","Final payment received → Payment Received → prepare and review deliverable → Release to Customer → Completed Scan Delivery or Document Delivery → Complete Request → Order Completed → Customer Experience Review.","must")
    elif title=="Document Intake and Provenance":
        add_heading(doc,"Safe administrator upload removal",3,anchor+"_safe_removal")
        add_bullets(doc,["An unreleased administrator/internal upload or accidental duplicate may be marked inactive with filename, operator, timestamp, and Timeline audit preserved.","Never destructively remove a customer source upload, Proof Completed Document, system/audit document, or released deliverable.","Use Withdraw Release for a released APS deliverable. Customer wording must be human-readable and never expose internal slugs such as customer_deliverable."])
    elif title=="Financial Reconciliation":
        add_heading(doc,"Live financial consistency",3,anchor+"_live_financials")
        add_bullets(doc,["After invoice, payment, refund, cancellation adjustment, or service conversion, refresh from the invoice/payment/refund ledger.","Verify total invoiced, invoice count, paid invoice count, paid to date, balance due, status, and Payment Received context agree.","Never require an unrelated status transition to make a financial summary current."])
    elif title=="Changing Service Type / RON to Mobile Conversion":
        add_heading(doc,"Same-request conversion workflow",3,anchor+"_conversion")
        add_bullets(doc,["Use Change Service / Convert Service only between RON and Mobile. Preserve the APS reference, customer, participants, documents, communications, Timeline, invoices, payments, refunds, appointment history, and Proof history.","Preview current totals, paid/net-applied amount, new legitimate total, additional balance or credit/refund, destination requirements, appointment impact, and Proof impact.","Enter a reason, confirm destination-service facts, and complete the conversion once. Do not cancel the whole APS request or rewrite prior invoices/payments.","Send Service Changed / Appointment Conversion after the approved conversion and verify customer Activity and destination-service Review Queue rules."])
        add_table(doc,["Worked example","Required result"],[("RON $45 paid → Mobile $70","Keep $45 history; create a $25 supplemental invoice; require Mobile address and scheduling."),("RON prepared/started → Mobile","Keep Proof/session history and earned/nonrecoverable cost evidence; supersede only future RON fulfillment."),("Mobile $70 paid → RON $45","Keep Mobile invoice/payment and appointment history; route legitimate $25 excess through credit/refund review; activate RON readiness."),("Equal-cost conversion","No invoice or refund; balance remains $0; current service/workflow changes with history preserved.")],[2.25,4.25])
        add_callout(doc,"Must Do","Preserve historical facts, require destination-service data, recalculate finances from the ledger, and verify conversion Timeline, Activity, template communication, and Review Queue.","must")
        add_callout(doc,"Do Not","Do not delete financial/document/provider history, invent a Proof fee, overwrite the prior appointment, create a replacement order, or activate Proof before normal readiness gates.","do_not")
def add_back(doc):
...
def add_matrix_chapter(doc,index,title,anchor,templates):
    ...
    elif index==2:rows=[("Under Review","Request created","Request + service details","Request Received","Automatic","None","Quote Ready / review","Review request"),("Under Review","Document needed","Admin review","Document Needed to Complete Quote","Manual Send Message","None","Under Review","Upload document"),("Quote Ready","Reviewed quote saved","Current quote version","Quote Ready","Send & Update","Customer-safe quote","Awaiting Approval","Review quote"),("Awaiting Payment","Quote approved / invoice open","Invoice ledger","Awaiting Payment","Automatic / Send Message","Invoice when maintained","Payment Received","Pay balance"),("Payment Received","Specific payment recorded","Payment + linked invoice","Payment Received","Workflow / Manual","Receipt/invoice if maintained","Fulfillment / final balance","View payment"),("Appointment Confirmed","Confirmed appointment saved","Appointment facts","Appointment Confirmed / Mobile Confirmation","Send & Update","None","Fulfillment","Attend / prepare"),("Service changed","Approved RON/Mobile conversion","Conversion + financial ledger","Service Changed / Appointment Conversion","Manual Send Message","None","Destination readiness","Review updated request"),("Final Payment Received","Final payment recorded","Payment + linked supplemental invoice","Payment Received","Workflow / Manual","Receipt/invoice if maintained","Delivery preparation","View payment"),("Document released","QC and release recorded","Released deliverable","Completed Scan / Document Delivery","Send Message","Exact released deliverable","Completion eligible","Download document"),("Cancelled","Admin decision persisted","Action + financial review","Cancellation Confirmed","Send & Update","Customer-safe confirmation","Refund state / closed","Review result"),("Completed","Completion gate passed","Service evidence + balance + release","Order Completed","Send & Update","None","Review eligible","View final state"),("Review eligible","Completed + released + no balance","Eligibility record","Customer Experience Review","Manual, once","None","Review Sent","Optional feedback")];headers=["Status / Event","Operator action","Evidence","Customer communication","Automatic / manual","Attachment","APS record / next","Customer sees"]
    ...
    if index==3:
        for start in range(0,len(rows),9):
            add_table(doc,headers,rows[start:start+9])
    else:add_table(doc,headers,rows)
