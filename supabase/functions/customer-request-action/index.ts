...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const existing = await rows(await db(`customer_action_requests?select=id&service_request_id=eq.${requestId}&action_type=eq.${actionType}&status=eq.pending&limit=1`));
    if (existing?.length) return json({ ok: true, duplicate: true, action: existing[0], status: actionType === "cancel" ? "cancellation_requested" : "reschedule_requested", reference_number: ref(requestId) });
    ...
    await db(`service_requests?id=eq.${requestId}`, { method: "PATCH", body: JSON.stringify({ customer_action_status: customerStatus, customer_action_reason: reason || null, cancellation_state: actionType === "cancel" ? "requested" : undefined, cancellation_requested_at: actionType === "cancel" ? new Date().toISOString() : undefined, reschedule_requested_at: actionType === "reschedule" ? new Date().toISOString() : undefined, proposed_appointment_at: proposed }) });
    await db("request_timeline_events", { method: "POST", body: JSON.stringify({ service_request_id: requestId, event_type: customerStatus, title: actionType === "cancel" ? "Cancellation requested" : "Reschedule requested", detail: reason || (proposed ? `Proposed appointment: ${proposed}` : "Customer submitted a request."), actor_type: "customer", visibility: "customer", metadata: { customer_action_request_id: action.id } }) });
    await db("review_queue_items", { method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify({ service_request_id: requestId, blocker_key: `${actionType}_requested`, title: actionType === "cancel" ? "Cancellation requested" : "Reschedule requested", detail: reason || "Customer action requires review.", target_tab: "overview", state: "open", source_object_type: "customer_action_request", source_object_id: action.id }) }).catch(() => null);
